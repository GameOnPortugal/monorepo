...
| M6 Jobs, lifecycle & media | 8 | 3 | M6.7 done (#19) — `scheduler/` deleted. M6.1 + M6.8 done — the in-process runner exists, is scheduling `week-screenshot-winner`, and reports per-run to an (optional) admin channel. **M6.3/M6.5/M6.6 can now register real jobs against a working runner** |
