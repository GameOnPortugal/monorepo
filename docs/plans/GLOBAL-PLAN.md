...
| **M10** | [Portal: identity, credit and the Hall of Fame](#m10--portal-identity-credit-and-the-hall-of-fame) | The portal ships without the community's logo, credits nobody, and its most promotable page is empty | — | 5 PRs — **two decisions are Luis's, see plan 09** |

```
M0 ──▶ M1 ──▶ M3 ──▶ M4 ──┬──▶ M5 ──▶ M6 ──▶ M8 ──▶ M10
                          └──▶ M9
...
| **M3.6** | ✅ **DONE (2026-08-21, #64)** — **Prisma 6 → 7, alone, as its own PR.** `prisma`/`@prisma/client` pinned to a matching `7.8.0` pair — not `^` ranges, and not "latest": `prisma`'s stable line tops out at `7.8.0` while `@prisma/client` had already moved to `7.9.1`, so "latest of each" would have paired mismatched versions. The generator moved from the deprecated `prisma-client-js` provider to the new `prisma-client` provider — confirmed empirically, not from docs alone, that `prisma-client-js` is a silent no-op under `prisma@7.8.0` (`prisma generate` reports success but writes nothing to `node_modules/.prisma/client`, and every `PrismaClient`/`Prisma` import then fails to resolve). The new provider writes plain TS source into `prisma/generated/` (gitignored, regenerated by `bunx prisma generate`) and unconditionally requires a driver adapter, ending the `binaryTargets` era outright: **`@prisma/adapter-mariadb` wraps the pure-JS `mariadb` npm driver, no native/musl binary at all**, so the old `["native", "darwin", "linux-musl-arm64-openssl-3.0.x", "linux-musl-openssl-3.0.x"]` pinning is gone, not rewritten. `inversify.config.ts`'s `new PrismaClient()` and the one test file that built its own clients (`AdLifecycleMigration.test.ts`, which used the now-removed `datasourceUrl` option) both now construct a `PrismaMariaDb` adapter first. A new `prisma.config.ts` carries the datasource URL and migrations path Prisma 7 moved out of `schema.prisma` — deliberately `process.env.DATABASE_URL` rather than the config module's own `env()` helper, because `env()` throws at config-load time if the variable is unset, and `docker/Dockerfile`'s builder stage runs `bunx prisma generate` at *build* time, before `DATABASE_URL` exists. A `tsconfig.json` `paths` alias (`@prisma/client` → `./prisma/generated/client.ts`) keeps the ~50 existing `from '@prisma/client'` call sites compiling and resolving unchanged, rather than a mechanical rewrite across that many files — and this alias had to be shipped into the **runtime** Docker image too (`COPY tsconfig.json`), not just used for `tsc`, since Bun's own module resolution at `bun run src/index.ts` reads it the same way; missing that copy is what the first Docker boot attempt caught (container started, then crashed on the first Prisma import). **A second, unplanned Dockerfile change was required**: `oven/bun:1.2.10-alpine`'s bundled Node-compat version is `v22.6.0`, and Prisma 7's own preinstall hook hard-requires `20.19+`/`22.12+`/`24.0+` — the build failed at `bun install` before ever reaching Prisma's code. Bumped both `FROM` lines to `oven/bun:1.3.14-alpine` (`v24.3.0`), which is also what local dev already runs, so this closes a dev/prod drift (CLAUDE.md's D5) instead of opening a third version into the mix. | [#7](../known-issues.md), [#13](../known-issues.md), [07 step 7](07-dependency-upgrades.md) | **Both extra checks the plan called for passed.** A cold `migrate deploy` against a freshly-created, empty database applied all 5 migrations cleanly from scratch and produced the same table set as the long-lived test database. A full production Docker image build (`--target runtime`) succeeded, and booting it against a real MariaDB with a syntactically-valid fake Discord token carried the container through DB-wait, `migrate deploy`, and full DI-container construction — `PrismaMariaDb` + `PrismaClient({ adapter })` resolve cleanly inside the real entrypoint — before failing only on `SELF_SIGNED_CERT_IN_CHAIN` reaching the real `discord.com` API, this environment's own TLS interception (already documented in `07-dependency-upgrades.md`'s data-quality caveat), unrelated to Prisma. **Environment note, not a defect in this PR**: this sandbox's `bun install` cannot reliably make live network requests at all — reproduced even for a single, isolated, cache-busted package fetch, and even inside a stock `oven/bun` container unrelated to this repo — so dependency resolution for verification was done via `npm install` against a local relay (Node's HTTP client reaches the real registry fine here; only Bun's own network client is affected) and migrated into `bun.lock` through Bun's own npm-lockfile-import feature, then `bun install --frozen-lockfile` was independently verified to succeed from that lockfile alone with no sandbox-specific state baked in. `bun.lock` in this PR is an ordinary lockfile. | **One transitive advisory had to be overridden.** Prisma 7 pulls `deepmerge-ts@7.1.5` through `prisma › @prisma/config`, which carries a **high** advisory (GHSA-ggr8-5vv4-36mx, stack exhaustion on recursive object graphs) — so `bun audit --audit-level=high` failed the build, correctly. Upgrading to Prisma **7.9.1** does *not* fix it; `@prisma/config` still resolves the vulnerable range. Pinned via a `deepmerge-ts: ^8.0.2` override instead. **That is a major bump on a package `@prisma/config` uses to load `prisma.config.ts`, which is on the production boot path**, so it was verified rather than assumed: `prisma generate`, `prisma migrate deploy` and the drift gate all still work on 8.0.2, plus 542 tests. Practical exposure was near zero either way — the only object graph `@prisma/config` merges is our own config file, not attacker input — but the gate exists because this repo went from 26 advisories to 0, and silently lowering it would give that number back. Remove the override once `@prisma/config` widens its range.
...
| **M5.7** | ✅ **DONE (2026-08-20, #57)** — **`wanted` subcommand** — restores an old-bot feature; lives in `📖anuncios` alongside sales, distinguished by colour and title. `WantedSubcommand.ts` is a deliberate near-duplicate of `SellSubcommand.ts` (same post-then-persist flow, same option shape) rather than a shared base class — `adType='wanted'` and the pt-PT copy are the only things that differ, and every other pair of sibling subcommands in this directory (`SoldAdSubcommand`/`BumpAdSubcommand`) is hand-duplicated the same way rather than factored. Confirmed the M5.5 prediction: `renderAdListing()` needed **zero** changes — it was already colour-coded off `ad.adType` (blue `#4199E7` for `wanted`). | [G3](../discord-bot-feature-gap.md), [01 T8](01-marketplace-overhaul.md) |
| **M5.8** | ✅ **DONE (2026-08-20, #57)** — **`list` improvements**: ephemeral, paginated, status-aware, working links. Replaced the M4.10 stopgap (cap at 25 fields, "N omitted" footer) with real Prev/Next pagination, following **M7.6's `/trophy rank` shape** (`RankPresenter`/`TrophyComponentHandler`) rather than inventing a second pagination style: `ListUserAdsPage` query + handler (clamp-then-fetch, same as `GetRankHandler`), `AdListPresenter` (embed + button building), and a `list-page` action added to the existing `MarketplaceComponentHandler` (`mkt` namespace — one handler per namespace, per M4.7). Page size 10, so even a 100-ad seller never gets near the 25-field cap. `AdRepository.findByUserId()` grew an optional `{limit, offset}` pagination window (omitting it keeps every existing caller — autocomplete, the lifecycle jobs — on the old unbounded behaviour) plus a new `countByUserId()`. Status-aware: each field shows 🟢 Activo / 🟡 Pendente de renovação / 💰 Vendido / ⚪ Expirado. A "View listing" link is only rendered when `message_id` is actually populated — the 28 orphaned M0.1 rows show "🔗 Sem publicação associada" instead of a broken link. | [01 T9](01-marketplace-overhaul.md) |
| **M5.9** | ✅ **DONE (2026-08-20, #57)** — **`search` subcommand** + `AdRepository.search()`/`countSearch()` — keyword / zone / type / condition / max price, filtered **entirely in the database** (never a JS `.filter()` over an unbounded fetch — same reasoning `ScreenshotRepository.findRequiringRelink()`'s doc comment gives, per this item's own evidence pointer). Shares `AdListPresenter`/`AdPage` with M5.8. **Design deviation from trophies' pagination, deliberate and written up here**: `list`'s Prev/Next buttons can encode their full state in the custom ID (a Discord snowflake + two small integers), but `search`'s free-text pt-PT `keyword`/`zone` filters cannot — Discord's custom ID caps at 100 characters and forbids `:`, and percent-encoding accented Portuguese text can **triple** its length, so trophies' "encode everything, survive a restart" approach would either silently truncate criteria (page 2 searching something subtly different from page 1 — a correctness bug) or throw on an ordinary two-word query. Instead, search criteria live in a new in-process `SearchCriteriaStore` (`Infrastructure/Bot/Discord/Component/Marketplace/`), keyed by a short opaque token embedded in the custom ID (`mkt:search-page:<token>:<page>`), with a 15-minute TTL. A stale/expired/post-restart click fails closed with an ephemeral "run the search again" reply rather than a stale or empty render — an acceptable trade-off for a marketplace snapshot (prices/stock change constantly), unlike a trophy leaderboard someone might page through days later. `CreateAdHandler` now also parses `price_cents` at creation time via the existing `AdPrice.ts` (previously only `EditAdHandler` did this, per M5.6's row) — otherwise a never-edited ad would be permanently invisible to `max_price`, which defeats the point of the filter existing. | [01 T10](01-marketplace-overhaul.md) |
| **M5.10** | ✅ **DONE (2026-08-20, #58)** — **Limits + admin override**: max 10 active ads per user; `ManageMessages` holders bypass ownership on `delete`/`sold`. `sold`'s admin override already existed (M5.6's `MarkAdSold.isAdmin`); this item brought `delete` in line with it the same way — `DeleteAd` grew an `isAdmin: boolean = false` (default, not required, so every existing call site/test stayed valid), `DeleteAdHandler`'s ownership check became `authorId !== userId && !isAdmin`, and `DeleteAdSubcommand` now reads `isGuildAdmin()` off the live interaction, exactly mirroring `SoldAdSubcommand`'s existing shape. The 10-ad limit (`Domain/Marketplace/AdLimits.ts`, a new `AdRepository.countActiveByUserId()`/`CountActiveAds` query) is checked in `SellSubcommand`/`WantedSubcommand` **before anything is posted to Discord**, not inside `CreateAdHandler` — post-then-persist (M0.1) means a limit enforced only at the write would refuse to save a listing that is already public, reproducing the exact orphaned-message failure mode M0.1 exists to prevent. `wanted` and `sell` ads share one active-ad pool per member, not separate caps. | [G2.3](../discord-bot-feature-gap.md), [01 T11](01-marketplace-overhaul.md) |
| **M5.11** | ✅ **DONE (2026-08-20, #58)** — **Item images re-hosted to MinIO at upload**, never linked from Discord's CDN. Built on M6.0's `MediaStorage`/`SafeImageFetcher` ports, not a new adapter, via a new `AdImageUploader` (`Infrastructure/Media/`). **One deliberate departure from `CreateScreenshotHandler`'s M6.2 shape**, written up on `AdImageUploader.ts` and `CreateAd.ts`: a screenshot's Discord attachment is a real file attachment, durable the moment Discord accepts the upload, so re-hosting it can safely happen *after* the message post (M6.2). A marketplace listing's photo is an **embed image URL** (`embed.setImage(url)`) — Discord never re-hosts that, it just points the client at whatever URL the message carries — so posting first with a raw Discord CDN URL would bake a 24-hour-lived link into the message from the start, precisely the bug M5.11 exists to avoid. So the re-host happens in `SellSubcommand`/`WantedSubcommand`, **before** `renderAdListing()`/`sendRichMessage()` ever runs, and `CreateAd` carries already-durable `images: string[]`, not a raw URL for `CreateAdHandler` to process later. Optional `image`/attachment options added to both `sell` and `wanted`; a non-image content type is refused before anything is posted. | [01 decision 5](01-marketplace-overhaul.md) — needs M2.3. **M6.0's `MediaStorage` port and `SafeImageFetcher` guard are ready to use** — build on those rather than a new adapter. |
...
| **M6.9** | ✅ **DONE (2026-08-23)** — **The 30-day expiry backstop.** M6.5 built the *courtesy* path (14 days idle → DM → 72h → expire) but never the *hard* one, so every route to expiry ran through a DM the owner has to receive. Production on 2026-08-23 showed the consequence: 5 `active` ads whose `expires_at` passed on **2025-05-10**, still listed on the public portal (which renders the column as "Expira"), because their single owner has DMs closed — `ads-lifecycle` had been reporting `considered: 5, changed: 0, skipped: 5, recipientsDmClosed: 1` once a day since it was switched on, and "skip and carry on" had no exit. Three parts: (1) **`expires_at` now means something while `active`** — `CreateAdHandler` sets creation + 30 days, `RenewAdHandler` sets a fresh 30 days instead of the `null` it used to clear to, `BumpAdHandler` pushes it out too (a bump is the owner saying "still available"); (2) **`AdRepository.findPastExpiry`** feeds a new `pastExpiry` bucket that `AdsLifecycleJob` expires directly, no DM, reason `past-expiry` — reusing the same idempotent `ExpireAd` command as the other three paths, so the message still comes down and the row is still never deleted; (3) a data migration (`20260823130000_active_ad_expiry_backfill`) gives pre-existing `active` rows a deadline measured from `bumped_at ?? createdAt`, so a long-idle row is *already* overdue rather than being handed a fresh 30 days it did nothing to earn. The four buckets are de-duplicated in `FindAdsDueForLifecycleActionHandler` (orphaned > pastExpiry > idle) — without it the job counts one ad twice and, worse, DMs "renew this within 72h" about an ad the same run has already expired. | Production `ads` + `job_runs` query, 2026-08-23 | Deliberately a **backstop, not a policy change**: a reachable owner is still prompted at day 14 and settled by day 17, well inside 30, so this only ever bites when the DM could not be delivered (closed DMs, member left, send threw, or the 5-recipients-per-run cap deferring indefinitely). **Revises decision 7 of plan 02** ("`expires_at` has no meaning while active") — that is precisely the hole the stuck ads fell through; the `pending_renewal` reply-deadline meaning is unchanged, and the migration deliberately does not touch those rows, since overwriting one would silently grant 30 extra days to someone already asked to answer within 72 hours. |
...
| ID | Item | Status |
| -- | ---- | ------ |
| **M8.1** | ✅ **DONE (2026-08-20, #53)** — Pulled from the Discord API for guild `818108848492773377` and vendored to `brand/`: `guild-icon-1024.png` (the flaming gamepad-skull mark) and `logo-lockup-2048.png` (mark + brush wordmark + platform glyphs, from `discovery_splash`). The guild has **no banner** (`banner: null`), so there is no wide hero asset — the lockup is the closest thing and is what the OG card is built from. **The palette is now measured rather than quoted**: every hex was sampled from the artwork, and all four face-button colours (`#4199E7` `#EA3223` `#8AFBCC` `#FFFD54`) are *exact* matches for what `00-overview.md` and `03-portal.md` already claimed — so those documents are confirmed, not merely assumed. Derived favicons (32/180/192/512) and a 1200×630 OG card into `portal/web/public/`, replacing the scaffold's placeholder SVG, and wired into `index.html` with OG/Twitter tags. Two derivation decisions worth keeping: the mark is composited onto `#060302` rather than left transparent (it is white line-art, so on a light tab or iOS home screen it would render blank), and the icon is cropped to the mark's bounding box first (611×765 inside an 836×836 frame — Discord's circular-crop padding costs a third of the linear resolution at 32px). **Still open: a vector trace.** This is detailed line-art and hand-authoring an SVG would produce something that merely *resembles* the logo, which is worse than an honest raster — a wrong logo damages more than a slightly soft one. It needs the original vector artwork or a real tracing tool. Recorded in `brand/README.md`. | **Superseded.** This cell previously said the assets could not be fetched from a text-only tool loadout and listed tracing + vendoring as the next agent's job. The fetch is done. What remains is only the vector trace, and the wordmark in `portal/web/src/components/Layout.tsx` is still set text rather than the mark — a one-line swap once someone decides whether the header should show the mark, the lockup, or stay typographic. Design tokens landed with M8.5 ([#48](https://github.com/GameOnPortugal/monorepo/pull/48)). |
...
| **M8.8** | Screenshots gallery + Hall of Fame. **Blocked on M6.3.** Thumbnails at ingest — a phone must not download a 4 MB PNG per grid tile. | ✅ **DONE ([#56](https://github.com/GameOnPortugal/monorepo/pull/56)); the thumbnails gap this row previously left open is now closed in [#63](https://github.com/GameOnPortugal/monorepo/pull/63).** M6.3 (relink recovery) landed weeks earlier, so this was unblocked. `/screenshots`: grid with a platform filter, a "carregar mais" button over a client-held pool (see below), and a keyboard-navigable `Lightbox` (Esc closes, arrow keys move, click-outside closes). `/screenshots/hall-of-fame` exists as its own route. **Broken images**: `LazyImage` swaps to a "Sem imagem" placeholder on `<img onError>`, covering the two dead 2022 Discord CDN links (622/624 re-hosted to MinIO) and any future ones — including a thumbnail request the endpoint below had to refuse — a row with a dead image still counts and still appears, it doesn't vanish from the grid or the total. **Hall of Fame is an honest placeholder, not a ranking** — verified against `discord-bot/src/Application/Query/Screenshot/GetScreenshotWinner/GetScreenshotWinnerHandler.ts` that the weekly winner is computed live from Discord reaction counts on each `WeekScreenshotWinnerJob` run and is **never persisted**: no `isWeeklyWinner`/`wonAt` column on `Screenshot`, no separate winners table. Building a real one needs a bot-side schema change to persist a winner flag/table at selection time — `discord-bot/prisma` was off-limits both times this page was built (a concurrent agent owned that tree each time). Fabricating a ranking from `createdAt` or reaction-less heuristics would misrepresent real winners as data — so the page says so plainly and links to Discord, where the real weekly announcement happens today. Recorded here as the concrete next step. **Thumbnails — closed in [#63](https://github.com/GameOnPortugal/monorepo/pull/63), option (a) from this row's own three-way choice, built as recommended (2026-08-21).** `GET /api/media/thumbnail?src=<origin URL>&w=<160\|320\|480>` on `portal-api` (`src/routes/media.ts`, `src/lib/thumbnails.ts`, `src/lib/mediaAllowlist.ts`): fetches the origin image — host, bucket path prefix and extension allowlisted to `media.game-on-portugal.pt`'s `gop-media` bucket, the same shape as the bot's `SafeImageFetcher`/`DiscordCdnAllowlist` but a fresh, separate implementation (the portal has no dependency on `discord-bot/src`, and never will — see `mediaAllowlist.ts`'s header) — resizes with `sharp` and re-encodes to WebP, and caches the result on disk under `THUMBNAIL_CACHE_DIR` (default `/data/thumbnails`, sharing the `portal_audit_data` volume M8.11 already added rather than minting a new one). `Screenshots.tsx`'s grid tiles now request the thumbnail via the new `thumbnailUrl()` helper; the `Lightbox` still opens the full-size origin image. `LazyImage`'s viewport gating stayed exactly as built — it's complementary (request-count), never a substitute for shrinking bytes, and its header comment was updated to say so precisely. **Measured against real production images, not fixtures** (fetched live off `media.game-on-portugal.pt`, through the actual built container, per the task brief's "prove it, don't assert it"): a 793,992-byte JPEG (`.../screenshots/019e8451-dbe7-7391-b036-f7cfbde78bee.jpg`) resized to **3,316 bytes** at `w=320` — a **239×** reduction; a 1,157,537-byte PNG (`.../019e6a4d-dd79-71a5-a221-380980def507.png`) resized to **4,184 bytes** — **277×**. A repeat request for the same `(src, w)` was served from the on-disk cache in **4 ms** vs. **366 ms** for the first fetch+resize+cache-write, byte-identical output confirmed by `cmp`. A non-media host (`evil.example.com`) is refused with 400 before any fetch is attempted (SSRF guard). A missing/renamed object 404s cleanly (`"source image not found"`) rather than hanging or 500ing — the path the two dead-link rows would hit if ever requested (in practice they never reach this endpoint at all: their stored `imageUrl` is still the original Discord CDN host, which fails the host allowlist itself, one step earlier). **`sharp` on Alpine/musl was verified, not assumed**: its own docs claim musl support since v0.32, but a claim is not a verification — the real `portal-api` image was built from the real `portal/api/docker/Dockerfile` and run, and `require('sharp')` plus an actual resize were exercised **inside that running Alpine/musl container** (confirmed via `docker run --platform linux/amd64`, matching the GitHub Actions build target), not only on the darwin-arm64 development machine, before it was trusted for this row. **Design decisions owned under the standing instructions' latitude:** - **A small fixed width set (`160`/`320`/`480`), never an arbitrary `?w=`** — an open resize endpoint is both a cache-blowup and a CPU-DoS vector; three widths cover the grid's tile sizes across breakpoints without either. - **The query string is dropped when normalising `src`** (`mediaAllowlist.ts`) before it is used as either a fetch URL or a cache key — otherwise `?anything=` mints unbounded distinct cache entries for the same underlying object, defeating the cache bound and the allowlist's intent at once. - **A hard cap on cache entry count** (`MAX_CACHE_ENTRIES` in `thumbnails.ts`, oldest-file eviction before every write) on top of the fixed-width bound — belt and braces: 624 images × 3 widths is small (a few tens of KB each, per the task brief), but the endpoint should not be able to grow its cache without bound if it is ever hit with many distinct *valid* media URLs. - **The cached WebP is served `Cache-Control: public, max-age=31536000, immutable`** — screenshots are never edited in place, so a given `(src, w)` pair's bytes are stable forever; correctness never depends on any invalidation path existing. - **Tests**: `portal/api/tests/mediaAllowlist.test.ts` (host/path/extension allowlist, SSRF bypass attempts, query stripping) and `portal/api/tests/media.test.ts` (route-level: resize+shrink, cache hit with no re-fetch, arbitrary width refused, non-media host refused with zero fetches, origin 404, oversized declared `Content-Length`, a non-decodable 200 body) — all against a fake `global.fetch`, no real network in `bun test`, matching this repo's existing convention. **Known limitation, written down rather than hidden**: the thumbnail cache shares M8.11's `portal_audit_data` volume rather than getting its own. Both are small and losing either is non-catastrophic for the cache (it regenerates lazily on the next request) — unlike the audit log, which is why that row, not this one, is the one that calls out backup coverage as a gap. |
...
| **M8.10** | Discord OAuth + admin shell, gated on guild membership **and `ManageMessages`** — the same definition of "admin" the bot uses. | ✅ **DONE ([#59](https://github.com/GameOnPortugal/monorepo/pull/59)).** `portal/api/src/lib/discordAuth.ts` is the portal's half of the same check `discord-bot/src/Domain/Bot/AdminCheck.ts`'s `isGuildAdmin()` makes, arrived at differently because the two services see different data: the bot reads `member.permissions` straight off a gateway interaction; the portal has no gateway connection or bot token at all (`portal/README.md`, "Schema ownership" — this service only ever had a DB connection before this), only the end user's own OAuth token, so it reads the equivalent `permissions` field `GET /users/@me/guilds` returns per guild and decodes the same `ManageMessages` bit (`0x2000`). **One faithfulness fix found by writing the test first**: discord.js's `PermissionsBitField#has()` — what `isGuildAdmin()` actually calls — treats `ADMINISTRATOR` as satisfying *any* permission check by default (`checkAdmin: true`), so a guild owner/administrator without the literal `ManageMessages` bit explicitly granted would pass the bot's check and fail a naively-ported portal one; `computeAdminStatus()` checks both bits for exactly this reason (see that function's doc comment). Sessions are a **signed, stateless cookie** (HMAC-SHA256 over a JSON payload, `src/lib/session.ts`), not a session table — the same "the bot owns the schema, don't reach for a migration" constraint M8.11's audit log hit (see that row) would have applied here too, so this sidesteps it entirely: the server verifies signature + expiry on every request and looks nothing up. Rotating `SESSION_SECRET` logs out every admin at once, which is the point. **Degrades safely**: `loadOAuthConfig()` returns `null` unless `DISCORD_CLIENT_ID`/`DISCORD_CLIENT_SECRET`/`SESSION_SECRET` are all set, and every `/api/auth/*`/`/api/admin/*` route checks it first and answers `503` rather than 500ing or crashing the process — verified by a whole test group (`degrades safely when OAuth is not configured`) that unsets the three vars and asserts the public routes are untouched, mirroring the bot's own `DISCORD_TOKEN`-unset → `InMemoryClient` contract (`AGENT.md` "Traps"). The admin shell (`portal/web/src/pages/admin/AdminLayout.tsx`) is a separate top-level route tree from the public `<Layout>`, not nested inside it, with its own three states (not configured / not authenticated, with `?error=forbidden`/`?error=oauth_failed` surfaced inline / authenticated) — `AdminAuthProvider` is scoped to just the `/admin` route so an anonymous visitor never fires an extra `/api/auth/me` request. **Tests, matching the task brief's explicit minimum**: `discordAuth.test.ts` — a non-member is refused, a member without `ManageMessages` is refused, an admin (and separately, an Administrator without the literal bit, and a member with `ManageMessages` in a *different* guild) is/isn't allowed, all pure-function, no network; `adminAuth.test.ts` — the same three outcomes again at the route level, driven through `buildApp()` with `global.fetch` swapped for a hand-rolled fake answering Discord's three endpoints (no mocking library, matching this repo's `tests/Helper/`-style convention), plus the degrade-safely group. **Two secrets Luis must create** (neither invented nor committed here — see `.env.example`'s "ADMIN OAUTH" section and `docs/operations.md`'s "Portal (M8)" section for the exact steps): `DISCORD_CLIENT_SECRET` (Discord Developer Portal → the Game On Portugal application → OAuth2 → Client Secret, plus adding the `/api/auth/callback` redirect URI there) and `SESSION_SECRET` (any random ≥32-byte value). `DISCORD_CLIENT_ID` is reused from the bot's existing var — it's not a secret, it's public in every OAuth URL. **Not yet verifiable against production**: `game-on-portugal.pt` has no public URL until M8.15's DNS cutover (the Caddy block exists in this repo but isn't applied to HTZ1 yet), so the OAuth redirect round trip can only be exercised locally/against the bare container IP today, not end-to-end against the real domain. |
| **M8.11** | Admin CRUD + audit log. | ✅ **DONE ([#59](https://github.com/GameOnPortugal/monorepo/pull/59)).** A new `src/repositories/admin/*.ts` (ads/screenshots/trophyProfiles/jobs) sits alongside the existing read-only `src/repositories/*.ts` rather than extending it — the public ones stay `findMany`/`findUnique`/`$queryRaw*`-only by convention (`portal/README.md`), the admin ones are the one place in `portal/api` allowed to write, and only reachable behind `requireAdmin` (`src/routes/admin.ts`). Writes are narrow and match what the schema can actually support without a migration: ads get `description`/`price`/`zone` edits, force-expire (`status: 'expired'`), and **soft** delete (`status: 'deleted'`, matching the existing lifecycle's four/five-value status set from M5.3 — never a hard `DELETE`); screenshots get a **hard** delete, because unlike `ads`, `schema.prisma`'s `Screenshot` model has no lifecycle column at all to soft-delete into (adding one is exactly the kind of `discord-bot/prisma` migration this agent was told not to make); trophy profiles get `isBanned`/`isExcluded` toggled (both columns already exist, written today by M7's `TrophiesSyncJob` — this is a manual override of the same two flags, not a new concept). **The audit log itself hit the task brief's flagged risk directly** — "if the audit log needs a table, that is a `discord-bot/prisma` migration you cannot make. Prefer a design that does not need one." It does need somewhere durable, so `src/audit/db.ts` uses a **private SQLite file this service owns outright**, via Bun's built-in `bun:sqlite` (zero new dependency, zero touch to the bot's MySQL schema or migration history), persisted on its own new Docker volume (`portal_audit_data`, `infrastructure/game-on-portugal.yaml`) so it survives redeploys. Every admin write (edit/expire/delete/flag-toggle) records `adminId`, `adminUsername`, `action`, `entityType`/`entityId`, a `detail` JSON blob, and a timestamp, in the same request as the mutation. **One real gap, written down rather than hidden**: this volume is **not** covered by the existing `gop-db-backup` container, which backs up the bot's MySQL schema only — noted in `docs/operations.md`'s new "Portal (M8)" section as a follow-up, not fixed here (new-in-this-PR infrastructure with no production history yet; wiring a second backup target is a distinct, reviewable change). Tests: `tests/admin/{ads,screenshots,trophyProfiles,jobs,auditLog}.test.ts`, against the real test database via a new `adminCookie()` test helper (signs a valid session directly — no OAuth round trip needed to test CRUD), asserting both the mutation's effect *and* that an audit row was written with the right actor. |
| **M8.12** | Admin jobs page, wired to M6.1's runner. | ✅ **DONE, with a scope departure from plan 03's original acceptance criterion ([#59](https://github.com/GameOnPortugal/monorepo/pull/59)).** `GET /api/admin/jobs` and the dashboard both read real `job_runs` rows (`job_name`/`last_run_at`/`status`/`summary`/`error`, M6.1) — that half is exactly the item as worded. **Not built: a "run this job now" trigger**, which plan 03's task table originally called for ("An admin can dry-run the winner job and see the result"). `job_runs` is written by the bot's own **in-process** scheduler; there is no HTTP surface on `discord-bot` for anything outside that process to invoke a job on demand, and building one is `discord-bot/src` work — explicitly out of this agent's tree (owned by a different, concurrently-running agent per the task brief) — and arguably deserves to be scoped and reviewed on its own regardless, since an admin-authenticated trigger endpoint on the bot is new attack surface, not a UI nicety. Per the standing instructions ("If it *is* a descope, that is a call for Luis — write it up rather than doing it"): this is recorded as exactly that, in `src/repositories/admin/jobs.ts`'s header and here. **Next agent's starting point**: either a narrow, admin-authenticated POST endpoint on the bot that enqueues/runs one named job, or a shared "requested run" signal the existing scheduler polls for (e.g. a nullable `requested_at` column on `JobRun` — which is, again, a `discord-bot/prisma` migration someone with access to that tree needs to make). |
| **M8.13** | SEO, OG cards, sitemap. | ✅ **DONE, analytics explicitly out of scope ([#59](https://github.com/GameOnPortugal/monorepo/pull/59)).** `portal/web/src/lib/seo.ts`'s `useDocumentHead()` overrides `document.title`/`og:title`/`og:description`/`og:image`/`og:url` for the current route and restores the site defaults (`index.html`'s M8.1 tags) on unmount — wired into all six public pages (Home, Marketplace, the per-ad `MarketplaceDetail`, Screenshots, Hall of Fame, Trophies); `MarketplaceDetail`'s card uses the ad's own name/description/first image once loaded, the site defaults while loading. **Documented limitation, not silently assumed away**: this is a client-rendered SPA with no server-rendering, so these tags only ever affect the in-browser `<title>`/meta — most link-unfurlers (Discord included) read `index.html`'s static tags without executing JS, which is exactly why M8.1 put the real OG image and copy there already. Per-page cards become load-bearing for actual link previews only if SSR/pre-rendering is added later; `seo.ts`'s header says so explicitly so this isn't mistaken for full per-page unfurling. `robots.txt` (`portal/web/public/robots.txt`, static — `Disallow: /admin`) plus a **live** `GET /sitemap.xml` (`portal/api/src/routes/seo.ts`, proxied by `gop-portal-web`'s nginx exactly like `/api/` and `/health` already are) listing the five static public routes plus up to 500 active ad detail URLs with `lastmod`, built through the same `publicAdsWhere()` visibility helper every public route uses so a soft-deleted/non-public ad can't leak into it. **Analytics**: plan 03's task table row #13 says "SEO, OG cards, sitemap, analytics," but this milestone's own item wording (and this agent's task brief) says only "SEO, OG cards, sitemap" — no analytics tool, consent banner, or tracking script was added. Treated as a genuine open decision for Luis, not an oversight: the community is EU-based (plan 03's privacy section already flags GDPR), so "which tool, and does it need a cookie-consent banner" is a product/legal call this agent isn't positioned to make unilaterally. |
| **M8.14** | Deploy + CI, documented in `operations.md`. | ✅ **DONE — mostly documentation, per the task brief's own framing ([#59](https://github.com/GameOnPortugal/monorepo/pull/59)).** Confirmed rather than assumed: `deploy.yml` already builds and pushes `joshlopes/game-on-portugal-portal-{api,web}` on every push to `main` (M8.2, PR #48) and `infrastructure/game-on-portugal.yaml` already runs both as `gop-portal-api`/`gop-portal-web` in the live stack — there was no missing pipeline to build. **One real gap found and closed**: the deploy's post-deploy health gate (`.github/actions/portainer-deploy`, the step that watches a container for a restart-loop after `PUT /api/stacks` — added after the 2026-08-20 bot incident that step's own header describes) only ever checked `gop-bot`. A portal container could have crash-looped through every deploy since M8.2 with the workflow reporting green the whole time — the exact failure mode that step exists to catch, just not extended to the two new containers when they joined the stack. `health-check-container` now accepts a space-separated list (`gop-bot gop-portal-api gop-portal-web`), checked within the same shared settle/window budget rather than one settle/window per container. **`docs/operations.md`** gained a new "Portal (M8)" section: the updated container list, exactly which two OAuth secrets are still missing from the Portainer stack env and how to add them, the audit-log volume and its (missing) backup coverage, the sitemap/robots routing, and — the one thing this item can't itself resolve — that there is **no public URL to verify any of this against** until M8.15's DNS cutover, so "verifying a deploy" for the portal today means checking the containers' own healthchecks over SSH, not loading `game-on-portugal.pt`. |
...
| **M9.7** | ✅ **DONE (2026-08-21, #66)** — **Privacy**: a single opt-out honoured by **both** the bot and the portal, a short privacy page, and a deletion request path that removes portal content too. | [03 decision 5](03-portal.md) | **Built as a new `PrivacySetting` table (`privacy_settings`), not a `public_opt_out` column duplicated onto `ads`/`screenshots`/`trophyprofiles`** — the shape `portal/api/src/repositories/visibility.ts`'s M8.3 scaffolding comments assumed, before this migration existed. Reason, written into `schema.prisma`'s model doc comment: none of the three content tables carries a real foreign key to a canonical "user" today (`author_id`/`author_id`/`userId` are un-enforced free-text Discord snowflakes), so there was no single place a member's decision already lived, and a member's opt-out should cover everything they have ever posted — and everything they post after — from one row, not one the bot has to remember to copy onto every future ad/screenshot/profile. One row per Discord member (`discordId String @id`, `publicOptOut Boolean @default(false)`), created lazily by the opt-out command; absence of a row means "opted in", so the ~13k+ pre-existing rows across `ads`/`screenshots`/`trophyprofiles` needed no backfill at all — purely additive `CREATE TABLE`, migration `20260821191431_add_privacy_settings`, verified with a cold `migrate deploy` against an empty database. **Bot**: `/privacy` (English top-level name, matching `screenshot`/`trophy`/`marketplace`; pt-PT via `setDescriptionLocalizations` and every reply string, same convention as M5.4) with three subcommands — `opt-out`/`opt-in` (`SetPrivacyOptOut` command/handler, `Domain/Privacy/PrivacyRepository.ts` → `OrmPrivacyRepository`), both ephemeral, and `delete-data` (GDPR erasure), which requires a `confirmar:APAGAR` string option before it does anything irreversible. **Deletion is a real hard delete, not the existing soft-delete path** — `AdRepository`/`ScreenshotRepository` gained a `deleteAllByAuthor()` method each (bypasses `ads`' `deleted_at` soft-delete entirely, since a soft-deleted row still has `description`/`price`/`author_id` sitting in the table), and `DeleteMemberDataHandler` deletes a member's `Trophies` rows before their `TrophyProfile` (no `onDelete: Cascade` on that relation, so the reverse order throws a foreign-key error). **Conservative default, not built, and written here per the task's own instruction**: erasure is immediate with no grace/undo period — GDPR's right to erasure does not admit a grace period as a compliant default, and Luis has not been asked whether he wants one; if he does, that is a new, separate command, not a change to this one. **Portal**: `repositories/visibility.ts` rewritten — `publicAdsWhere`/`publicScreenshotsWhere` are now `async`, querying `optedOutDiscordIds()` once per call and excluding those authors via `OR author_id IS NULL` (a null/unattributed author can never match an opt-out row, so it must stay visible — not trusted to Prisma's own `notIn`-on-nullable behaviour); `publicTrophyProfileFilter()` stays synchronous but the exclusion is folded into its existing raw-SQL fragment as a `NOT IN (SELECT discordId FROM privacy_settings WHERE publicOptOut = true)` subquery, since that query was already `$queryRaw`. Every caller (`repositories/ads.ts`/`screenshots.ts`/`stats.ts`, `routes/seo.ts`'s sitemap) updated to `await` the now-async builders — `tsc` catching every missed call site was the actual regression net here. **Fails closed by construction, not by a try/catch**: none of the three functions catches — a `privacy_settings` query failure propagates out of the repository, out of the route handler (Hono's `onError` → 500), and the fail-closed test in `portal/api/tests/privacy.test.ts` proves this by monkeypatching `prisma.privacySetting.findMany` to reject and asserting the ads route returns 500 rather than 200-with-everything. **Privacy page**: `portal/web/src/pages/Privacy.tsx`, pt-PT, static (no API call — the portal is read-only by convention, so a self-service opt-out/erasure form here would have to write through the bot anyway; the page explains what's shown and points at the two `/privacy` bot subcommands instead), linked from the footer next to Admin. Tests: `discord-bot/tests/Integration/{Application/Write/Privacy,Infrastructure/Bot/Discord/SlashCommand/Privacy}` (opt-out/opt-in round-trip, GDPR erasure hard-deletes across all four tables and leaves other members untouched, confirmation-phrase gate, ephemeral pt-PT replies) and `portal/api/tests/privacy.test.ts` (opted-out author absent from all three public read paths, reappears on opt-in, fail-closed on a broken check). |

---

## M10 — Portal: identity, credit and the Hall of Fame

**Goal**: the portal looks like Game On Portugal, credits the people whose
content it publishes, and its "most promotable page on the site" (plan 03)
stops being an apology.

**Raised by Luis, 2026-08-21.** Full design, evidence and the two open
decisions: [`09-portal-community-identity.md`](09-portal-community-identity.md).

Three findings shape this milestone, all verified against the tree on
2026-08-21:

1. **The logo was never lost.** M8.1 vendored it (`brand/`); M8 then only ever
   derived it into favicons and the OG card. Every on-page brand appearance is
   the *text* wordmark. Web-only fix.
2. **Votes, author names and winners are not in the database.** Votes are
   `trophy_plat` reactions counted live off Discord each run; the author is a
   bare snowflake with no name; the weekly winner is computed in memory and
   announced, never persisted. Items M10.5/M10.6/M10.8 are therefore *bot work
   first*, and cannot be solved inside `portal/` alone.
3. **The historical winners are recoverable, faithfully.** The old bot's
   `scripts/screenshot-winners.js` (`git show
   481661e^:old-discord-bot/scripts/screenshot-winners.js`) announced every
   winner into `#screenshots` in a fixed parseable shape — `Parabéns <@id>
   ganhaste o screenshot da semana com <name>. Plataforma: <platform>.` plus
   the winning message's permalink. Parsing those beats recomputing from
   today's reaction counts, which would retroactively hand old contests to
   whoever accumulated reactions since.

**Depends on**: M8 throughout (this extends it), M6.3's channel-history paging
for M10.7's backfill.

| ID | Item | Evidence | Notes |
| -- | ---- | -------- | ----- |
| **M10.1** | ✅ **DONE (2026-08-22)** — **Put the mark on the site.** Derive a transparent mark + wide lockup from `brand/` into `portal/web/public/brand/`; use them in the header, hero and footer. | [09](09-portal-community-identity.md), [brand/README.md](../../brand/README.md) | Keep the text wordmark as the accessible name — an image-only header would regress the one thing M8.5 got right. No SVG trace: M8.1's "an honest raster beats an approximate vector" stands. **Shipped** as `portal/web/src/components/BrandMark.tsx` + two derived PNGs (`public/brand/mark-64.png`, `mark-512.png`), used in the header (32px, beside the wordmark), the hero (128px) and the footer (24px). **The one non-obvious bit is the derivation**: `brand/README.md` said the mark was "white line-art on transparency", but the source file is *fully opaque* — already composited over `#060302`. That is a premultiplied-over-near-black image, so alpha was recovered as each pixel's distance from that background (**max channel, not luminance** — luminance keys the four saturated face buttons down to ~half-transparent) and the colour by un-premultiplying (without which the antialiased edges composite back darker and the mark reads thinner than the artwork). Real alpha rather than an opaque `#060302` crop because the mark also has to sit on `--color-surface` `#120D0A` in M10.3's placeholder, where an opaque crop shows a visible dark square. Recipe written up in `brand/README.md`. No wide lockup was derived — the header pairs the mark with the existing *text* wordmark, which stays crisp at any zoom and remains the accessible name, so a second raster of the same words would have been strictly worse. Verified in a real browser against the built output: all three `<img>` load (`naturalWidth` 51/405/51, not broken). |
| **M10.2** | ✅ **DONE (2026-08-22)** — **Un-link `/admin` from the footer.** | Luis, 2026-08-21 | Reverses M8.10's deliberate "discreet, not hidden" call in `Layout.tsx`. That comment is not wrong — OAuth + `ManageMessages` is the actual control — but an unadvertised URL is strictly less log noise and Luis has asked for it. `/admin` keeps working; only the link goes. **Shipped.** One correction to the framing above, recorded so nobody re-derives it as a security win: removing the link protects nothing by itself — the gate is Discord OAuth + `ManageMessages` and it is unchanged. What it buys is that the login form is no longer advertised to every passer-by and crawler, so the auth logs stop collecting drive-by attempts. `/admin` verified still reachable and still showing the Discord gate. |
| **M10.3** | ✅ **DONE (2026-08-22)** — **Branded placeholder** in `LazyImage` instead of the bare "Sem imagem" tile. | Luis, 2026-08-21 | Covers the 2 known-dead 2022 CDN links and any refused thumbnail. **Shipped.** The two states sharing this branch were kept visually distinct: not-yet-in-viewport renders an empty surface (it is about to be replaced), only genuinely-missing renders the mark at 25% opacity + "Sem imagem". Rendering the mark for the viewport-gating case would make a scrolling gallery flash logos on every tile. |
| **M10.4** | ✅ **DONE — identity half (2026-08-22)**; vote half deferred. **Persist the author's Discord identity.** | [09](09-portal-community-identity.md) | **Shipped, but not in the shape this row proposed, and deliberately narrower.** Two changes of design, both driven by evidence read out of production on 2026-08-22 rather than from the plan: (1) **a `discord_profiles` table, not `screenshots.author_name`.** Production holds 624 screenshots from **83 distinct authors** and 71 ads from 31 — 109 across both. One row per *member* means a rename or a new avatar is one UPDATE instead of a sweep over every row that member ever posted, and `ads` (which `repositories/ads.ts` records as having the identical gap) is credited for free without a second backfill. (2) **avatars are in**, re-hosted through the existing M6.0/M6.2 MediaStorage pipeline and keyed by Discord's *avatar hash* so no public URL carries a member's snowflake (`MediaKey.ts`'s "no user IDs" rule) — see plan 09 decision 1 for the reversal. **The backfill needs no message parsing at all**: 0 rows have a null `author_id`, so it is 83 `GET /users/{id}` calls, and `DiscordProfilesSyncJob` orders never-synced authors first so the job *is* the backfill (`jobs:run discord-profiles-sync --limit=200`). GDPR erasure drops the cached profile row too. **Deferred, and still open**: `vote_count`/`votes_synced_at` + `ScreenshotVotesSyncJob`. Luis asked for names, avatars and winners; a sync job over 624 rows for a number only M10.6 needs is a separate item, and the winner row stores the count it won with, which is what the UI actually renders. **M10.6 therefore stays blocked.** |
| **M10.5** | ✅ **DONE (2026-08-22)** — **Credit + Discord permalink** in the gallery (vote count deferred with M10.4's vote half). | Luis, 2026-08-21 | Exposes a display name, a re-hosted `avatarUrl` and a derived `messageUrl` — never `author_id`/`channel_id`/`message_id` themselves, so `repositories/screenshots.ts`'s privacy header stays true. **Plan 09 decision 1 resolved (Luis, 2026-08-22)**: names yes ("it's fine to put their usernames in the public page ... this is a small community"), and then **avatars too** on Luis's follow-up call the same day — option C had been rejected for *hot-linking someone else's image*, which re-hosting is precisely not. **Shipped** as `portal/api/src/repositories/credit.ts` (the single place that decides what "crediting a member" means) plus `components/AuthorCredit.tsx`. Three states all render rather than hide: name+avatar, name+monogram (member uses Discord's default, or the re-host failed), and no author at all — an uncredited screenshot is still listed, because hiding it would make the gallery's own count lie. `Privacy.tsx` updated in the same PR, as plan 09 requires. |
| **M10.6** | **Sort by votes / most recent.** | Luis, 2026-08-21 | Server-side once `vote_count` exists. Today's gallery is `createdAt desc` with no control. Blocked on M10.4. |
| **M10.7** | ✅ **DONE (2026-08-22)** — **Winner persistence + historical backfill.** New `ScreenshotWinner` model (table `screenshot_winners`); `WeekScreenshotWinner` writes a row as it announces; `screenshots:backfill-winners` reconstructs history from the old announcements. | finding 3 above | Rows carry `source: 'announced' \| 'inferred'` so the page can be honest where an announcement was deleted. **Finding 3 verified against the live channel, then corrected by the real run (2026-08-23)**: `#screenshots` holds 1,444 messages back to 2021-04-02, containing **139 bot-authored winner announcements**, of which **137 resolve to a surviving screenshot row**; the other 2 name screenshots the old bot itself hard-deleted (its script deleted any row whose message it could not fetch). **The archive is 137 weeks, spanning 2022-01-17 to 2024-10-14, with 28 distinct winners.** An earlier count of "178 announcements, 41 unresolved" was quoted in the PR body for #93 and is wrong: that probe matched any message containing "Parabéns" plus a permalink, so it also counted members quoting or congratulating. The 137 was right throughout — it came from the bot-filtered probe — but the denominator was not. Every recovered row is `inferred` with a **null `voteCount`**: the old bot's announcement format never stated a reaction count, which is exactly why that column is nullable rather than defaulted to 0. Three implementation calls worth keeping: the **permalink**, not the prose, is the load-bearing part of an announcement (`message_id` is what maps it back to a row, and 0 production rows have a null one); the week is dated from the **winning screenshot's** `createdAt` via a new `weekWindowContaining()` rather than the announcement's timestamp (`computeWeekWindow` looks *backwards* by design and would be off by one week for any mid-week reference); and an `inferred` row **never overwrites an `announced` one**, or re-running the backfill after a few normal weeks would downgrade first-hand rows and make the portal's disclaimer lie. Dry by default; `--apply` writes. |
| **M10.8** | ✅ **DONE (2026-08-22)** — **Hall of Fame, for real** — winner gallery + the **winner trophy** badge. | [03](03-portal.md) pages table, Luis 2026-08-21 | **Shipped**: `/api/screenshots/winners` + a rewritten `HallOfFame.tsx` (year filter, lightbox, credit line), and a `WinnerBadge` on the gallery tile and in the lightbox. Two honesty affordances are as much the point as the gallery: an `inferred` entry is marked as archival, and when a decided week is **withheld** — its screenshot deleted, or its author opted out — the page states how many rather than presenting a quietly shorter history as the whole story (`total` counts decided weeks; the list only contains the visible ones). The badge is yellow-on-outline, never one of the four platform accents, which mean *platform*. Week labels and the year filter are formatted in **Europe/Lisbon**, not the visitor's timezone — a contest week is defined in Lisbon local time, so a Sunday-23:59:59.999 boundary formatted elsewhere reads as Monday, and a week closing on 31 December would be filed under the wrong year. The winner trophy is a **badge derived from `ScreenshotWinner`, not a row in `trophies`** — that table is strictly scraped PSN platinums summed into the leaderboard, and a synthetic community award would corrupt the one number `/trophy rank` and the portal both promise is real. Blocked on M10.7. |
| **M10.9** | ✅ **DONE (2026-08-22)** — **PSN profile links** on the leaderboard → `https://psnprofiles.com/<psnProfile>`. | Luis, 2026-08-21 | Same base URL `PsnProfilesTrophySource` already scrapes, so a working link is guaranteed for any profile the sync job can read. **Shipped** — `psnProfileUrl()` in `Trophies.tsx`, `encodeURIComponent`'d because the column is un-validated free text at the DB level. Opens in a new tab with an `↗` affordance and an `sr-only` "(abre o perfil no PSNProfiles)". A null `psnProfile` stays plain text rather than becoming a link to `psnprofiles.com/null`. |
| **M10.10** | ✅ **DONE (2026-08-22)** — **"Como participar" explainers** for the rank system and the marketplace. | Luis, 2026-08-21 | pt-PT (cross-cutting rule 1). Both pages end in a Discord CTA — the portal is read-only, so every action lives in the bot. **Shipped** as `pages/ComoParticipar.tsx` (route `/como-participar`, added to the header nav), covering ranking, the M10.11 section, marketplace, screenshots and the privacy opt-out. **Every command name, limit and threshold was read out of the bot's source rather than remembered** — subcommands from `SlashCommand/{Trophy,Marketplace}`, `MAX_ACTIVE_ADS_PER_USER = 10`, `AD_LIFECYCLE_IDLE_DAYS = 14`, `AD_LIFECYCLE_RESPONSE_HOURS = 72`, and the rarity→TP ladder from `TrophyPoints.ts` — because a page of confidently wrong instructions is worse than no page; the file header lists the sources so they can be re-checked when they change. Deep-linked from `/trophies` and `/marketplace`. Client-side hash navigation does not scroll on its own (the fragment is only honoured on a document load), so the page scrolls to `location.hash` itself — without that the two leaderboard entry points would have silently behaved like links to the top of the page. Verified end-to-end in a browser: the "Não apareço aqui, porquê?" link lands the section at 80px, clearing the sticky header exactly as `scroll-mt-20` intends. |
| **M10.11** | ✅ **DONE (2026-08-22)** — **"Porque é que não apareço no ranking?"** — the banlist ask, redesigned. | Luis, 2026-08-21 | **Recommended against publishing a named banlist** (plan 09 decision 2): it is search-indexable and permanent, and most `isExcluded` rows mean "left the server" or "PSN profile went private", not "misbehaved" — `TrophiesSyncJob` sets the flag for all three. The explainer serves the stated goal ("people come to discord to cry it over") *and* the larger group whose profile broke for a boring reason. Aggregate counts fine, names not. If Luis wants the named list, it belongs behind the existing admin auth — `AdminTrophyProfiles.tsx` already shows exactly that to moderators. **Shipped** as the `#fora-do-ranking` section. **The evidence got stronger while building it**: `TrophiesSyncJob` sets `isBanned` with the literal reason *"sem rank visível no PSNProfiles"* — i.e. the profile went private, was renamed or was deleted — and `hasLeft` for Discord error 10007 (left the server). So `isBanned` does not mean the member did anything wrong; it is a legacy misnomer inherited from the old bot's "if we can't read the profile it's probably banned" heuristic (the comment is still at the top of the recovered `screenshot-winners.js`). A public named list would therefore have implied misconduct about people whose profile is merely private — which settles the question rather than just weighing it. The four reasons are ordered most-common first, the first two are self-service, and the genuinely-moderated case routes to Discord to appeal. |
...
| M3 Dependencies & container | 8 | 8 | **complete** — #11 #20. `bun audit` **26 advisories → 0** and the gate is blocking again. #64 — **M3.6 Prisma 6→7**, standalone PR, verified with a cold `migrate deploy` and a full Docker build+boot |
...
| M5 Marketplace overhaul | 11 | 11 | **complete** — #29 (M5.3 lifecycle columns + migration), #35 (M5.1 route to `#anuncios`, M5.2 delete removes the message and soft-deletes the row), #49 (M5.4 pt-PT copy pass, M5.5 listing embed + buttons — first `ComponentHandler`, M5.6 shared sold/bump/edit handlers + edit modal), #57 (M5.7 `wanted`, M5.8 `list` pagination, M5.9 `search`), #58 (M5.10 limits + admin override on delete, M5.11 images to MinIO via a new `AdImageUploader`) |
| M6 Jobs, lifecycle & media | 10 | 10 | **complete** — #19 (M6.7), #30 (M6.0 `MediaStorage` + S3/MinIO), #31 (M6.4 winner hardening), #32 (M6.1 runner + M6.8 reporting), #37 (M6.2 fix-at-source, M6.3 relink recovery), #40 (M6.5 `ads:lifecycle`, M6.6 `ads:reconcile`). Every scheduled job the plan asked for now exists and is registered against a working runner. M6.9 (2026-08-23) closed the one gap production found afterwards: expiry that does not depend on a DM arriving |
...
| M8 Community portal | 15 | 14 | **[#48](https://github.com/GameOnPortugal/monorepo/pull/48)** — M8.2 (release-please + `infrastructure/game-on-portugal.yaml` + CI wiring), M8.3 (`portal/api` scaffold, read-only over the bot's schema, 9 tests), M8.5 (`portal/web` scaffold: tokens, routing, one representative page). **[#55](https://github.com/GameOnPortugal/monorepo/pull/55)** — M8.1 (real brand assets vendored, favicons + OG card derived; vector trace still open, noted in that row). **[#56](https://github.com/GameOnPortugal/monorepo/pull/56)** — M8.4 (shared normalisation module, portal-side only — bot-side adoption recorded as follow-up in that row), M8.6 (Home: live stats via a new `/api/stats`, real strips into the pages below), M8.7 (Marketplace grid/filters/detail), M8.8 (Screenshots gallery + Lightbox, and an honest Hall of Fame placeholder — winner history is never persisted anywhere in the schema, see that row; that row's thumbnails gap was closed separately, see below), M8.9 (Trophies leaderboard — the plan's "data frozen" notice is correctly *not* present, since M7 landed the same night). **[#59](https://github.com/GameOnPortugal/monorepo/pull/59)** — M8.10 (Discord OAuth admin shell, same `ManageMessages`-or-`Administrator` definition as the bot, stateless signed-cookie sessions, degrades to 503 with nothing configured), M8.11 (admin CRUD over ads/screenshots/trophy-profiles + an audit log — a private `bun:sqlite` file, not a MySQL table, per the task brief's own schema-ownership constraint), M8.12 (jobs page, **read-only by design** — no bot-side trigger surface exists, written up as a real descope rather than built around), M8.13 (per-page SEO + live `sitemap.xml`; analytics explicitly out of scope, flagged as Luis's call), M8.14 (confirmed the deploy pipeline already covered the portal; closed one real gap — the post-deploy health gate only watched `gop-bot`, now watches all three containers — and documented the rest in `docs/operations.md`). Remaining, untouched: **M8.15** (the plan-04-phase-4 domain cutover — a deliberate production DNS/Caddy change for Luis, not this agent's to make). |
| M9 Feature gap & dead weight | 7 | 7 | **complete** — #33 — M9.2/M9.3/M9.4 **done**, seven dead models and their tables dropped (migration `20260820102655_drop_dead_models`, guarded and verified against a production copy). M9.1 redesigned onto AutoMod. #44 — **M9.5 verified** (env vars were already clean; fixed the stale trap note in `AGENT.md` instead), **M9.6 done** (`old-discord-bot/` deleted, all references fixed). #66 — **M9.7 done** (privacy: a new `PrivacySetting` table rather than a flag duplicated across three tables, `/privacy opt-out`/`opt-in`/`delete-data` in the bot, fail-closed filtering in the portal, a pt-PT privacy page) |
| M10 Portal: identity, credit & Hall of Fame | 11 | 10 | **#86** — M10.1 (brand mark on the site), M10.2 (`/admin` unlinked), M10.3 (branded placeholder), M10.9 (PSNProfiles links), M10.10 ("Como participar"), M10.11 ("Porque não apareço no ranking?"). **This session** — M10.4 (author identity: a `discord_profiles` table + `DiscordProfilesSyncJob`, avatars re-hosted to MinIO; the `vote_count` half deliberately deferred, see that row), M10.5 (credit + permalink in the gallery), M10.7 (winner persistence + a backfill that recovered **137 weeks** from announcements going back to April 2021), M10.8 (Hall of Fame, for real). Remaining: **M10.6** (sort by votes), still blocked on M10.4's deferred vote half |
| **Total** | **104** | **101** | |
