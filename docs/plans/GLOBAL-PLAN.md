...
| **M4.3** | ✅ **DONE (2026-08-20)** — **Registration overhaul.** Guild-scoped dev path; hash the command set and skip the `PUT` when unchanged; add `setDefaultMemberPermissions`, `setContexts([Guild])` and `setIntegrationTypes`. | C3, C5, [06 §4](06-discord-api-modernisation.md), A2 | Built with one deliberate deviation from the item's original wording: the dev guild is **`DISCORD_DEV_GUILD_ID`** (new, optional, `Config/env.ts`), **not** `DISCORD_GUILD_ID` as this row originally said — `DISCORD_GUILD_ID` (`DiscordChannels.ts`) had since become "the production guild" with its own verified default, so reusing it here would risk registering guild-scoped commands into production, silently shadowing the global ones. Unset (the default) means global registration, same as production; `resolveCommandRegistrationTarget()` in `src/Domain/Bot/CommandRegistration.ts` picks `Routes.applicationGuildCommands` vs `Routes.applicationCommands` from it, and `DiscordBot.registerSlashCommands()` logs which scope is live on every boot. Command-set hash (`hashCommandSet()`, same file) is kept in a small file store (`src/Infrastructure/Bot/Discord/CommandSetHashStore.ts`) under the container's `$HOME`, keyed per registration scope — **not** a DB table (no schema change) and explicitly **not** guaranteed to survive a real redeploy (the bot image has no persistent volume; only MariaDB/MinIO get one), so a genuine version bump still re-registers once. That's an accepted trade-off, documented in the store's own comment: it still kills the redundant PUT on same-container restarts (crash loops, manual Portainer restarts, local `bun run --watch`), which was the unconditional cost before this. A failed hash write never blocks registration (logged as a warning) and a hash read failure always falls through to writing — "when in doubt, write" per this item's own instruction. All four top-level commands got `setContexts(InteractionContextType.Guild)` + `setIntegrationTypes(ApplicationIntegrationType.GuildInstall)` + explicit `setDefaultMemberPermissions(null)` — see **M1.10**, same edit, done together. Tests: `tests/Integration/Domain/Bot/CommandRegistration.test.ts`, `tests/Integration/Infrastructure/Bot/Discord/CommandSetHashStore.test.ts`, plus a `builder()` assertion in each of the four `*SlashCommand.test.ts` files. |
...
| M6 Jobs, lifecycle & media | 9 | 2 | M6.7 done (#19) — `scheduler/` deleted. M6.0 done — `MediaStorage` port + S3/MinIO adapter, unblocks M5.11/M6.2/M6.3. **M6.1 is now the only route to any scheduled work running at all** |
