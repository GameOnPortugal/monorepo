...
| **M8.15** | ✅ **DONE (2026-08-21)** — **Plan 04 phase 4**: `game-on-portugal.pt` and `www` now resolve to HTZ1 and serve the portal. | — Done in this order on purpose: **Caddy vhost first, DNS second**. The host had no vhost for the apex (`infrastructure/caddy/…` is a documentation file, not applied config), so flipping DNS first would have taken the site *down* rather than moved it. Backed the Caddyfile up to `~/caddy-backups/` on HTZ1 — the directory is group-read-only so a sibling backup is not possible — added the block, `caddy validate` returned **Valid configuration** with all 29 site blocks intact, reloaded, and spot-checked an unrelated vhost still served 200 before touching the zone. At OVH: the four GitHub Pages apex A records became one A to `195.201.192.35`, and `www` had to be **delete-and-recreate** rather than update, since `fieldType` cannot be changed from CNAME to A. MX, SRV and autoconfig were left untouched, so **email is unaffected**. All record edits were staged and applied with a **single zone refresh**, which is what makes the cutover atomic — OVH does not publish record changes until the zone is refreshed. **Diagnostic worth keeping**: for about an hour afterwards `dig` returned the new IP while `curl` still fetched the old site. That was not propagation — `dig` queries the configured nameserver directly while `curl` goes through `getaddrinfo` and the OS cache. The proof the cutover had completed was the **certificate**: Let's Encrypt issued `CN=game-on-portugal.pt` (valid to 2026-11-19), which it cannot do unless the domain publicly resolved to HTZ1 for the ACME challenge. Verified serving `lang="pt-PT"` and live `/api/stats` from two independent networks. **Still open, now unblocked** — deleting `webpage/`, archiving `GameOnPortugal/gameonportugal.github.io` (checked: not archived, last push 2021-11-11) and dropping the stale `_github-pages-challenge` TXT record: see [08-decommission-and-cleanup.md](08-decommission-and-cleanup.md) C2/C3, which deliberately keeps the TXT record until a Pages rollback is definitively ruled out. |
