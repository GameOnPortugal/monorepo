...
| M7 Trophies | 8 | 7 | #24 (M7.1 PSNProfiles source, M7.2 points ladder), then M7.3 (`trophies:sync`, scheduling opt-in via `TROPHIES_SYNC_ENABLED` plus a moderation safety valve), M7.4 (live rank on `/trophy check`), M7.5 (both `/trophy create` URL shapes), M7.7 (`trophies:fix-old` backfill), and this PR's **M7.6** (rank presentation parity — guild emojis with a unicode fallback, `trophies` component-handler pagination). Remaining: **M7.8** (announcements through the bot instead of the webhook — design already decided, see [Decisions taken](#decisions-taken), just not built) |
