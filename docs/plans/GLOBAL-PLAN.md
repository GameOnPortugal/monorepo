...
| **M4.7** | **Component routing infrastructure.** A `ButtonHandler` interface in `Domain/Bot/`, a `TYPES.ButtonHandler` multi-binding, and dispatch on `isButton()` / `isStringSelectMenu()` / `isModalSubmit()` / `isAutocomplete()` by `customId` prefix (`mkt:sold:<adId>`). | C4, [01 T6](01-marketplace-overhaul.md) | **Always re-check ownership server-side from the row — never trust the `customId`.** Anyone can click anyone's button. `BotExecutor` handles chat-input only today. **M6.5 (2026-08-20) added a second consumer that needs this**: its renewal DM ships a `Renovar` button with `customId` scheme `mkt:renew:<adId>`, and `RenewAdHandler` (`Application/Write/Marketplace/RenewAd/`) already does the server-side ownership recheck (`ad.authorId !== userId` off the row, never the customId) — but nothing dispatches to it yet. When this item is built, wire `mkt:renew:<adId>` to `RenewAdHandler` (via `CommandHandlerManager`, passing the interaction's own `userId` and the resolved `channelId`) alongside whatever `mkt:sold:<adId>` needs. |
...
| M4 Discord API modernisation | 10 | 8 | #27 (M4.1 typed interaction layer, M4.2 deferReply, M4.10 embed limits), #28 (M4.4 lifecycle, M4.5 REST-only guild client, M4.6 cache/sweepers, M4.9 attachment ingest), #34 (M4.3 registration). Remaining: **M4.7 component routing** and **M4.8 autocomplete** — M5.5/M5.6/M6.5's buttons and M7.6's pagination are all waiting on M4.7 |
