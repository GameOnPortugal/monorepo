...
| ID | Item | Evidence | Notes |
| -- | ---- | -------- | ----- |
| **M10.1** | **Put the mark on the site.** Derive a transparent mark + wide lockup from `brand/` into `portal/web/public/brand/`; use them in the header, hero and footer. | [09](09-portal-community-identity.md), [brand/README.md](../../brand/README.md) | Keep the text wordmark as the accessible name — an image-only header would regress the one thing M8.5 got right. No SVG trace: M8.1's "an honest raster beats an approximate vector" stands. |
| **M10.2** | **Un-link `/admin` from the footer.** | Luis, 2026-08-21 | Reverses M8.10's deliberate "discreet, not hidden" call in `Layout.tsx`. That comment is not wrong — OAuth + `ManageMessages` is the actual control — but an unadvertised URL is strictly less log noise and Luis has asked for it. `/admin` keeps working; only the link goes. |
| **M10.3** | **Branded placeholder** in `LazyImage` instead of the bare "Sem imagem" tile. | Luis, 2026-08-21 | Covers the 2 known-dead 2022 CDN links and any refused thumbnail. |
...
| **M10.5** | **Credit + Discord permalink + vote count** in the gallery. | Luis, 2026-08-21 | Exposes `authorName`/`voteCount`/derived `messageUrl` — never `author_id`/`channel_id`/`message_id` themselves, so `repositories/screenshots.ts`'s privacy header stays true. **Blocked on plan 09 decision 1** (how much identity goes public); recommendation there is name + permalink. Privacy page must be updated in the same PR. |
