...
| M6 Jobs, lifecycle & media | 9 | 6 | M6.7 (#19) `scheduler/` deleted; M6.0 (#30) `MediaStorage` port + S3/MinIO adapter; M6.1 + M6.8 the in-process runner, scheduling `week-screenshot-winner` and reporting per-run to an (optional) admin channel. **M6.5 + M6.6 (2026-08-20)** the `ads:lifecycle` and `ads:reconcile` jobs, registered against the M6.1 runner — no schema change, grace-capped first run, see their rows for the design. Remaining: **M6.2/M6.3** (screenshot message-id repair, still blocked on nothing but time) |
