...
| M9 Feature gap & dead weight | 7 | 4 | #33 — M9.2/M9.3/M9.4 **done**, seven dead models and their tables dropped (migration `20260820102655_drop_dead_models`, guarded and verified against a production copy). #TBD — **M9.1** built (AutoMod mechanism + starter config + `automod:apply`), not applied to the live guild — see its row for what Luis still has to decide. M9.6 schema half done; `old-discord-bot/` removal still open |
| **Total** | **93** | **63** | |
