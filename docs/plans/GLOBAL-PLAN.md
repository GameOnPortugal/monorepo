...
| **M10.4** | **Persist screenshot votes + author display name.** `screenshots.vote_count`/`votes_synced_at`/`author_name`, captured at ingest and refreshed by a new `ScreenshotVotesSyncJob`; one-off backfill of the 624 legacy rows. | [09](09-portal-community-identity.md) | Migration required (cross-cutting rule 5). Cadence is deliberately **not** "every row, nightly" — see plan 09 decision 3. |
| **M10.5** | **Credit + Discord permalink + vote count** in the gallery. | Luis, 2026-08-21 | Exposes `authorName`/`voteCount`/derived `messageUrl` — never `author_id`/`channel_id`/`message_id` themselves, so `repositories/screenshots.ts`'s privacy header stays true. **Plan 09 decision 1 resolved (Luis, 2026-08-22): display name + permalink, no avatar** — "it's fine to put their usernames in the public page ... this is a small community". Only M10.4 (the data) still blocks this. Privacy page must be updated in the same PR. |
...
| **M10.7** | **Winner persistence + historical backfill.** New `ScreenshotWinner` model; `WeekScreenshotWinnerJob` writes a row when it picks a winner; `screenshots:backfill-winners` reconstructs history from the old announcements. | finding 3 above | Rows carry `source: 'announced' \| 'inferred'` so the page can be honest where an announcement was deleted. Migration required. |
| **M10.8** | **Hall of Fame, for real** — winner gallery + the **winner trophy** badge. | [03](03-portal.md) pages table, Luis 2026-08-21 | The winner trophy is a **badge derived from `ScreenshotWinner`, not a row in `trophies`** — that table is strictly scraped PSN platinums summed into the leaderboard, and a synthetic community award would corrupt the one number `/trophy rank` and the portal both promise is real. Blocked on M10.7. |
