...
| 2 | **Route ads to `📖anuncios`.** Add `CommunityChannels.MARKETPLACE` + `DiscordChannels` entry; post via `GuildClient`, not `interaction.reply`. | Running the command in any channel posts the listing in `📖anuncios` only |
| 3 | **Delete removes the Discord message.** Extend `GuildClient` with `deleteMessage`; call it from the delete path; tolerate an already-deleted message. | Deleting an ad removes both row and message; deleting twice does not throw |
